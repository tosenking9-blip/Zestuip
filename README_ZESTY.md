...
## Match Control Add-on
- Added a dashboard-owned Match Control panel.
- Lone Wolf, Battle Royale, and alternating Mix modes are selectable.
- Start/stop and pause/resume state controls.
- Add dashboard-simulated players and update their X/Y/Z position.
- Directional movement controls and administrator teleport within the dashboard simulation.
- Live match state refreshes every 2 seconds.
- Match state is stored separately in `match_state.json`, so the existing user/device databases are not modified by this add-on.
- This panel intentionally does not emit packets to or manipulate a third-party game client.
