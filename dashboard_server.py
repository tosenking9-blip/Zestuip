...
async def handle_match_state(request):
    require_admin(request)
    return web.json_response(match_control.snapshot())

async def handle_match_mode(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    try:
        s = match_control.set_mode(data.get("mode"))
        audit_event("match_mode", {"mode": s["mode"]})
        return web.json_response(s)
    except Exception as e:
        return web.json_response({"error": str(e)}, status=400)

async def handle_match_running(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    return web.json_response(match_control.set_running(bool(data.get("running"))))

async def handle_match_pause(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    return web.json_response(match_control.set_paused(bool(data.get("paused"))))

async def handle_match_player(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    try:
        pid, p, s = match_control.upsert_player(data.get("player",""), data.get("x",0), data.get("y",0), data.get("z",0))
        return web.json_response({"player":pid,"state":p,"match":s})
    except Exception as e:
        return web.json_response({"error":str(e)}, status=400)

async def handle_match_move(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    try:
        return web.json_response(match_control.move_player(data.get("player",""),data.get("dx",0),data.get("dy",0),data.get("dz",0)))
    except Exception as e:
        return web.json_response({"error":str(e)}, status=400)

async def handle_match_teleport(request):
    require_admin(request); _check_csrf(request)
    data = await request.json()
    try:
        return web.json_response(match_control.teleport_player(data.get("player",""),data.get("x",0),data.get("y",0),data.get("z",0)))
    except Exception as e:
        return web.json_response({"error":str(e)}, status=400)

async def handle_match_clear(request):
    require_admin(request); _check_csrf(request)
    return web.json_response(match_control.clear_players())

    app.router.add_get("/api/admin/match", handle_match_state)
    app.router.post("/api/admin/match/mode", handle_match_mode)
    app.router.post("/api/admin/match/running", handle_match_running)
    app.router.post("/api/admin/match/pause", handle_match_pause)
    app.router.post("/api/admin/match/player", handle_match_player)
    app.router.post("/api/admin/match/move", handle_match_move)
    app.router.post("/api/admin/match/teleport", handle_match_teleport)
    app.router.post("/api/admin/match/clear", handle_match_clear)
